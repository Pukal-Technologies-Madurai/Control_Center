...
export const randomString = (length) => {
    const randomStr = crypto.randomBytes(Math.ceil(length / 2)).toString("hex").slice(0, length);
    return randomStr;
};
export const encryptPasswordFun = (text) => {
    const cipher = crypto.createCipher('aes-256-cbc', process.env.passwordKey);
    let encrypted = cipher.update(text, 'utf8', 'hex');
    encrypted += cipher.final('hex');
    return encrypted;
  };
  
  export const decryptPasswordFun = (encryptedText) => {
    const decipher = crypto.createDecipher('aes-256-cbc', process.env.passwordKey);
